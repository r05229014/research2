...
def VGG_3D():
    print("Build model!!")
    model = Sequential()
    model.add(Convolution3D(64,3, strides=(1,1,1), activation='relu', padding='same',kernel_initializer='random_uniform',bias_initializer='zeros', input_shape=(70,32,32,5)))
    model.add(Convolution3D(64,3, strides=(1,1,1),activation='relu', padding='same',kernel_initializer='random_uniform',bias_initializer='zeros'))
    ...
    for i in range(15):
        model.add(Dense(128, activation = 'linear',kernel_initializer='random_uniform',bias_initializer='zeros'))
        model.add(BatchNormalization())
        model.add(Dropout(0.2))
...
filepath="../../model/test9/weights-improvement-{epoch:03d}-{loss:.3e}.hdf5"
sgd = optimizers.SGD(lr=0.3, decay=1e-9, momentum=0.9, nesterov=True)
model.compile(optimizer = sgd, loss='mean_squared_error')
checkpoint = ModelCheckpoint(filepath, monitor='loss', 
                            save_best_only=False, mode='min', period=5)

model.fit(X,y, batch_size=16, epochs=400, shuffle=True, callbacks = [checkpoint])
